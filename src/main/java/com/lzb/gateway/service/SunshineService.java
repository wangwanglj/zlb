...
public class SunshineService {
    ...
    public void setIpQueue() throws SocketException, ExecutionException, InterruptedException {
        ...
        log.info("扫描sunshine服务耗时: " + (System.currentTimeMillis() - currentTimeMillis));
    }
    ...
    private void scanIp(String ip) {
        try {
            ...
            if (address.isReachable(TIMEOUT_MS)) {
                ...
                // 获取注册信息
                String namedCerts = jsonObject.getString("named_certs");
                NamingService namingService = nacosDiscoveryProperties.namingServiceInstance();
                registerInstance(namingService, ip, Integer.parseInt(sunshinePort), StringUtils.isEmpty(namedCerts));
                log.info("sunshine服务地址: {} namedCerts:{} ", sunshineUrl, namedCerts);
            }
